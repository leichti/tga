...
class Trial:
    ...
    def initialize(self):
        ...
        # filter spikes from h2o generator
        # selected_m = self.df[(self.df["h2o"]>0) & (self.df["gas1"]==0)]["m"]
        # selected_m = selected_m.iloc[500:]
        # filtered_m = median_filter(selected_m, size=1000)
        # self.df["m"].loc[selected_m.index] = filtered_m
        self.df["m"] = savgol_filter(self.df["m"], 200, 3)
...
if __name__ == "__main__":
    ...
    trial = ReductionTrial("befesa/others/529_T03.txt")
    trial.detect()

    t, dmdt2 = trial.get("dmdt2", "t")
    plt.plot(t, dmdt2)
    t, dmdt = trial.get("dmdt", "t")
    plt.plot(t, dmdt)
    plt.show()

    import sys
    sys.exit()
    h2file = H2File("befesa/others/2022-04-05-09_03_52_T03.csv")
    h2file.load()
    ...
    t, gas1 = trial.get("gas1", "t")
    effective_h2 = h2.f(t)
    h2_consumption = gas1 - effective_h2
    h2_consumption = 1000*h2_consumption/(60*22.4) # from ml/min to umol/s
    dmdt_oxygen = -dmdt/16*1000 # from mg/s to umol oxygen per s
    dmdt_zno = -1000*dmdt/(16+65)


    plt.add(t, dmdt_oxygen, label="umol O/s")
    #plt.plot(t, dmdt_zno, label="umol ZnO/s")
    plt.add(t - 35, h2_consumption, label="umol H2/s")
    #plt.plot(t, effective_h2)
    #plt.plot(*trial.get("gas1", "t"))
    #plt.plot(h2file.h2)
    #plt.plot(h2.reactor.h2_model, linestyle="dashed")
    plt.legend()
    plt.show()
